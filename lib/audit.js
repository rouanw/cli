  ...
  return Bluebird.all([
    ...
    if (args[0] === 'fix') {
      ...
      if (!auditResult.metadata) {
        const err = new Error('The registry returned no audit data')
        err.code = 'ENOAUDITDATA'
        err.message = auditResult.message ? err.message + ': ' + auditResult.message : err.message
        throw err
      }
